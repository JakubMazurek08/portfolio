...
interface TextProps {
    ...
    variant?: 'h1' | 'p' | 'button' | 'header' | 'special' | 'special2' | 'special3';
}
...
export const Text = ({children, variant}: TextProps) => {
    ...
    }else if (variant === 'special3'){
        return <h1 className={`${spaceGrotestBold.className} text-[24px]/[50px] md:text-[32px] lg:text-[48px] text-white`}>{children}</h1>;
    }
    return <p className={`${spaceGrotest.className} text-[16px] sm:text-[20px] text-lightGray`}>{children}</p>;
}